...
**v2.2.0** — vedi `CHANGELOG.md` per lo storico delle versioni. La
...
- **Piano**: giorni → pasti (Pranzo/Cena, estendibile) → alimenti
  previsti. Più piani per Casa; si crea vuoto, da un **template**, o
  clonando la settimana precedente.
- Compilare da template porta nel piano le **categorie attese per
  pasto** (es. Lunedì Pranzo → sezioni Carboidrati + Verdura): ogni
  sezione ha il proprio "+", che propone solo gli alimenti della
  dispensa compatibili con quella categoria — si riempie con calma,
  non tutto insieme al momento della compilazione.
- **Dispensa**: cosa la Casa mangia normalmente (non un inventario di
  ...
  un alimento in più categorie compare in ogni gruppo pertinente.
- **Template** (sotto-vista propria): si dà un nome e si costruisce
  giorno per giorno quali pasti servono e quali categorie sono attese
  per ciascuno — scrittura immediata, come nel resto dell'app.
...
- **Impostazioni**: le categorie alimento (condivise tra tutte le
  Case) si aggiungono ed eliminano da qui.
