...
/**
 * Aggiorna nome e categorie di un alimento esistente — stesso id, quindi
 * ogni riferimento già esistente (in un piano, in una checklist "Cosa mi
 * manca"...) continua a puntare allo stesso alimento, ora corretto: non
 * serve eliminare e ricreare, e non si perde nessuno storico.
 */
export async function updateAlimento(alimentoId, casaId, nome, categoriaIds) {
  const { error: erroreNome } = await db.from(ALIMENTI_TABLE).update({ nome }).eq("id", alimentoId);
  if (erroreNome) throw erroreNome;

  const { error: erroreRimozione } = await db.from(ALIMENTI_CATEGORIE_TABLE).delete().eq("alimento_id", alimentoId);
  if (erroreRimozione) throw erroreRimozione;

  if (categoriaIds?.length) {
    const righe = categoriaIds.map((categoriaId) => ({ alimento_id: alimentoId, categoria_id: categoriaId, casa_id: casaId }));
    const { error: erroreCategorie } = await db.from(ALIMENTI_CATEGORIE_TABLE).insert(righe);
    if (erroreCategorie) throw erroreCategorie;
  }
}
...
/** Aggiunge uno o più alimenti a un pasto in un colpo solo, opzionalmente sotto una sezione-categoria. */
export async function addAlimentiAPasto(pianoPastoId, casaId, alimentoIds, categoriaId = null) {
  const righe = alimentoIds.map((alimentoId) => ({
    piano_pasto_id: pianoPastoId, casa_id: casaId, alimento_id: alimentoId, categoria_id: categoriaId,
  }));
  const { error } = await db.from(PIANO_PASTO_ALIMENTI_TABLE).insert(righe);
  if (error) throw error;
}
...
/**
 * Crea un template con la struttura già pronta: 7 giorni × N pasti (uno
 * per ogni tipo pasto, es. Pranzo e Cena), esattamente come un piano.
 * Pranzo e Cena esistono sempre per ogni giorno: non è una scelta da
 * fare — se non ci si mette nessuna categoria, semplicemente quel pasto
 * non è pianificato per quel giorno. Nessun "+ pasto" da premere 14 volte.
 */
export async function createTemplate(casaId, nome) {
  ...
  const righeGiorni = Array.from({ length: 7 }, (_, giorno) => ({
    template_id: template.id, casa_id: casaId, giorno_settimana: giorno,
  }));
  const { data: giorni, error: erroreGiorni } = await db
    .from(TEMPLATE_GIORNI_TABLE).insert(righeGiorni).select("id");
  if (erroreGiorni) throw erroreGiorni;

  const righePasti = giorni.flatMap((giorno) =>
    tipiPasto.map((tipo) => ({
      template_giorno_id: giorno.id, casa_id: casaId, tipo_pasto_id: tipo.id,
    }))
  );
  const { error: errorePasti } = await db.from(TEMPLATE_PASTI_TABLE).insert(righePasti);
  if (errorePasti) throw errorePasti;

  return template.id;
}

/** Rinomina un template esistente — non tocca in alcun modo i piani già compilati da esso. */
export async function renameTemplate(templateId, nome) {
  const { error } = await db.from(TEMPLATE_PIANI_TABLE).update({ nome }).eq("id", templateId);
  if (error) throw error;
}

/** Aggiunge una o più categorie a un pasto del template in un colpo solo. */
export async function addCategorieAPasto(templatePastoId, casaId, categoriaIds) {
  const righe = categoriaIds.map((categoriaId) => ({
    template_pasto_id: templatePastoId, casa_id: casaId, categoria_id: categoriaId,
  }));
  const { error } = await db.from(TEMPLATE_PASTO_CATEGORIE_TABLE).insert(righe);
  if (error) throw error;
}
